...
import { PeripherialOracleTest, TestERC20 } from '../../typechain'
...
describe('OracleLibrary', () => {
  ...
  let oracle: PeripherialOracleTest
  ...
  const oracleTestFixture = async () => {
    ...
    const oracleFactory = await ethers.getContractFactory('PeripherialOracleTest')
    const oracle = await oracleFactory.deploy()

    return {
      tokens: tokens as TestERC20[],
      oracle: oracle as PeripherialOracleTest,
    }
  }
