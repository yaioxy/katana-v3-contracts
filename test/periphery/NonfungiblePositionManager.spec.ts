import { abi as IKatanaV3PoolABI } from '../../artifacts/src/core/interfaces/IKatanaV3Pool.sol/IKatanaV3Pool.json'
...
describe('NonfungiblePositionManager', () => {
  let wallets: Wallet[]
  let wallet: Wallet, proxyAdmin: Wallet, treasury: Wallet, other: Wallet
  ...
  let nftSigner: Wallet

  let loadFixture: ReturnType<typeof waffle.createFixtureLoader>

  before('create fixture loader', async () => {
    wallets = await (ethers as any).getSigners()
    ;[wallet, proxyAdmin, treasury, other] = wallets
  ...
  beforeEach('load fixture', async () => {
    ...
    nftSigner = await (ethers as any).getSigner(nft.address)
  })
  ...
  describe('#createAndInitializePoolIfNecessary', () => {
    ...
    it('works if pool is created but not initialized', async () => {
      ...
      await factory.connect(nftSigner).createPool(tokens[0].address, tokens[1].address, FeeAmount.MEDIUM)
      const code = await wallet.provider.getCode(expectedAddress)
    ...
    it('works if pool is created and initialized', async () => {
      ...
      await factory.connect(nftSigner).createPool(tokens[0].address, tokens[1].address, FeeAmount.MEDIUM)
      const pool = new ethers.Contract(expectedAddress, IKatanaV3PoolABI, wallet)
  ...
  describe('multicall exit', () => {
    ...
    async function exit({
      ...
      return nft.multicall([decreaseLiquidityData, collectData])
    }
  ...
  describe('fees accounting', () => {
    ...
    describe('10k of token0 fees collect', () => {
      ...
      it('expected amounts', async () => {
        ...
        expect(nft1Amount0).to.eq(2085)
        expect(nft1Amount1).to.eq(0)
        expect(nft2Amount0).to.eq(6256)
      ...
      it('actually collected', async () => {
        ...
        await expect(
          ...
          .to.emit(tokens[0], 'Transfer')
          .withArgs(poolAddress, wallet.address, 2085)
          .to.not.emit(tokens[1], 'Transfer')
        await expect(
          nft.collect({
            tokenId: 2,
          ...
          .to.emit(tokens[0], 'Transfer')
          .withArgs(poolAddress, wallet.address, 6256)
          .to.not.emit(tokens[1], 'Transfer')
      })
    })
  })
