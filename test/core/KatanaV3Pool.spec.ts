...
import { TestKatanaV3SwapPay } from '../../typechain/TestKatanaV3SwapPay'
...
import { TestKatanaV3ReentrantCallee } from '../../typechain/TestKatanaV3ReentrantCallee'
...
describe('KatanaV3Pool', () => {
  let wallet: Wallet, proxyAdmin: Wallet, treasury: Wallet, positionManager: Wallet, other: Wallet
  ...
  let pool: MockTimeKatanaV3Pool

  let swapTarget: TestKatanaV3Callee
  ...
  before('create fixture loader', async () => {
    ;[wallet, proxyAdmin, treasury, positionManager, other] = await (ethers as any).getSigners()
    loadFixture = createFixtureLoader([wallet, proxyAdmin, treasury, positionManager, other])
  })

  beforeEach('deploy fixture', async () => {
    ;({ token0, token1, token2, factory, governance, createPool, swapTargetCallee: swapTarget } = await loadFixture(poolFixture))
    ...
    governanceSigner = await (ethers as any).getSigner(governance.address)
  })
  ...
  describe('#mint', () => {
    ...
    describe('after initialization', () => {
      ...
      beforeEach('reset fee protocol to 0', async () => {
        await pool.setFeeProtocol(0, 1)
      })
      ...
      it('protocol fees accumulate as expected during swap', async () => {
        ...
        let token0ProtocolFees = await token0.balanceOf(treasury.address)
        let token1ProtocolFees = await token1.balanceOf(treasury.address)
      ...
      it('positions are protected before protocol fee is turned on', async () => {
        ...
        await pool.setFeeProtocol(1, 6)
        token0ProtocolFees = await token0.balanceOf(treasury.address)
        token1ProtocolFees = await token1.balanceOf(treasury.address)
        expect(token0ProtocolFees).to.eq(0)
        expect(token1ProtocolFees).to.eq(0)
      })
  ...
  async function initializeAtZeroTick(pool: MockTimeKatanaV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('miscellaneous mint tests', () => {
    ...
    it('collect fees within the current price after swap', async () => {
      ...
      const lowerTick = -tickSpacing * 1000
      const upperTick = tickSpacing * 1000
  ...
  describe('limit orders', () => {
    ...
    beforeEach('reset fee protocol to 0', () => pool.setFeeProtocol(0, 1))
    ...
    describe('fee is on', () => {
      beforeEach(() => pool.setFeeProtocol(1, 6))
      it('limit selling 0 for 1 at tick 0 thru 1', async () => {
        await expect(mint(wallet.address, 0, 120, expandTo18Decimals(1)))
          .to.emit(token0, 'Transfer')
          .withArgs(wallet.address, pool.address, '5981737760509663')
  ...
  describe('#feeProtocol', () => {
    ...
    describe('#collectProtocol', () => {
      ...
      it('already collected fees', async () => {
        await pool.setFeeProtocol(1, 6)
        ...
        let token0ProtocolFees = await token0.balanceOf(treasury.address)
        let token1ProtocolFees = await token1.balanceOf(treasury.address)
        expect(token0ProtocolFees).to.eq('83333333333333')
        expect(token1ProtocolFees).to.eq(0)
      })

      it('fees collected are similar between token0 and token1', async () => {
        await pool.setFeeProtocol(1, 5);
        ...
        let token0ProtocolFees = await token0.balanceOf(treasury.address)
        let token1ProtocolFees = await token1.balanceOf(treasury.address)
        expect(token0ProtocolFees).to.eq('100000000000000')
        expect(token1ProtocolFees).to.eq('99999999999999')
      })
    })
  ...
  describe('#flash', () => {
    beforeEach('allow flash loan', async () => {
      await factory.connect(governanceSigner).toggleFlashLoanPermission()
      expect(await factory.flashLoanEnabled(), "flash loan is enabled").to.be.true
    })
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestKatanaV3ReentrantCallee')
      ).deploy()) as TestKatanaV3ReentrantCallee
      await governance.setRouter(reentrant.address)
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestKatanaV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestKatanaV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestKatanaV3SwapPay
      await governance.setRouter(underpay.address)
