...
import { KatanaV3PoolSwapTest } from '../../typechain/KatanaV3PoolSwapTest'
...
describe('KatanaV3Pool arbitrage tests', () => {
  ...
  before('create fixture loader', async () => {
    ;[wallet, proxyAdmin, treasury, positionManager, arbitrageur] = await (ethers as any).getSigners()
    loadFixture = createFixtureLoader([wallet, proxyAdmin, treasury, positionManager, arbitrageur])
  })

  for (const feeProtocol of [0, 6]) {
    describe(`protocol fee = ${feeProtocol}/10;`, () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, proxyAdmin, treasury, positionManager, arbitrageur]: Wallet[]) => {
            const fix = await poolFixture([wallet, proxyAdmin, treasury, positionManager, arbitrageur], waffle.provider)
