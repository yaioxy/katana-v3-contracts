...
describe('KatanaV3Pool gas tests', () => {
  ...
  for (const feeProtocol of [0, 1]) {
    describe(feeProtocol > 0 ? 'fee is on' : 'fee is off', () => {
      ...
      const gasTestFixture = async ([wallet, proxyAdmin, treasury, positionManager, other]: Wallet[]) => {
        const fix = await poolFixture([wallet, proxyAdmin, treasury, positionManager, other], waffle.provider)
        ...
        const governanceSigner = await (ethers as any).getSigner(fix.governance.address)
        ...
        await pool.connect(governanceSigner).setFeeProtocol(feeProtocol, 6)
