...
describe('KatanaV3Pool', () => {
  ...
  let pool0: MockTimeKatanaV3Pool
  let pool1: MockTimeKatanaV3Pool
  ...
  let swapTargetCallee: TestKatanaV3Callee
  let swapTargetRouter: TestKatanaV3Router
  ...
  beforeEach('deploy first fixture', async () => {
    ;({ token0, token1, token2, factory, createPool, swapTargetCallee, swapTargetRouter, governance } = await loadFixture(
      poolFixture
    ))
    ...
    ): Promise<[MockTimeKatanaV3Pool, any]> => {
